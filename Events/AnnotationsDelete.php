...
class AnnotationsDelete extends AbstractEventHandler
{
    public static function getSubscribedEvents(): array
    {
        return [Events::ANNOTATIONS_DELETE];
    }

    public function __invoke(...$params): void
    {
        $details = $this->utility->extractEventDetails($params[1]);
        $log = "Deleted an annotation for site id {$details['params']['idSite']}.";
        $detailedLog = $this->utility->getDetails($details['params']);

        $this->logAudit('Annotations', 'delete', $log, $detailedLog);
    }
}
