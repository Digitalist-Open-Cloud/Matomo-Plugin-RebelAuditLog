...
class TagManagerPublishContainerVersion extends AbstractEventHandler
{
    public static function getSubscribedEvents(): array
    {
        return [Events::TAG_MANAGER_PUBLISH_CONTAINER_VERSION];
    }

    public function __invoke(...$params): void
    {
        $details = $this->utility->extractEventDetails($params[1]);
        $log = "A container version was published for container id {$details['params']['idContainer']} in environment {$details['params']['environment']} for site id {$details['params']['idSite']}.";
