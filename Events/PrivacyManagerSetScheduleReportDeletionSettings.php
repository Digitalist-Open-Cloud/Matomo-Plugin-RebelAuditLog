...
class PrivacyManagerSetScheduleReportDeletionSettings extends AbstractEventHandler
{
    public static function getSubscribedEvents(): array
    {
        return [Events::PRIVACY_MANAGER_SET_SCHEDULE_REPORT_DELETION_SETTINGS];
    }

    public function __invoke(...$params): void
    {
        $details = $this->utility->extractEventDetails($params[1]);
        $days = $details['params']['deleteLowestInterval'];
        if ($days == "1") {
            $interval = 'day';
        } elseif ($days == "7") {
            $interval = 'week';
        ...
        $type = gettype($days);
        $log = "Schedule old data deletion to every {$interval}.";
